...
export function Ticker() {
  ...
  return (
    <div className="overflow-hidden py-8 border-y border-white/[0.06] bg-white/[0.015]">
      <div className="flex gap-15 items-center animate-ticker w-max">
        {doubled.map((item, i) => (
          <span
            key={i}
            className="text-[13px] text-white/30 whitespace-nowrap font-medium"
          >
            <strong className="text-white/70">{item.name}</strong> {item.type}
          </span>
        ))}
      </div>
    </div>
